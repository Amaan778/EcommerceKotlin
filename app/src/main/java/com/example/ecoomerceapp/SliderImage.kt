...
class SliderImage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val geti=intent.getStringExtra("product_name")
        name.text=geti

        val idget=intent.getStringExtra("id")
                ...
                storage.putFile(it1).addOnCompleteListener {
                    if (it.isSuccessful) {
                        storage.downloadUrl.addOnSuccessListener { uri ->
                            ...
                            val update= mapOf(
                                "img1" to uri.toString()
                            )

                            db.collection("products").document(id).update(update)
                            val intent=Intent(this,SliderImage2::class.java)
                            intent.putExtra("products",title)
